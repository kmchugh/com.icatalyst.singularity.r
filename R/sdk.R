...
Singularity <- R6::R6Class("Singularity",
  ...
  private = list(
    ...
    #' @field  the uri to redirect the user to in order to start the redirect flow
    login_redirect_uri = NULL
  ),


  active = list(
    #' @field the custom message handler created to allow redirection from shiny
  ...
  public = list(
    ...
    #' @params redirect_uri the uri to redirect the user to in
    ...
    #' This is specifically for shiny
    #' @params session the session object from shiny
    shiny_redirect = function(session) {
      session$sendCustomMessage(private$message_handler, private$message_handler)
    }
  )
)
